...
//document.onmousedown=function(event){ 
	...
	if(event[setting.hot_key]==true&&event.button==2&&event.target.tagName.toLowerCase()=='img'&&event.target.src!=null){
		if(event.target.src.match(/^data:.*?;base64,/)!=null)alert('Sorry, this image\'s url is a base64 string. For all the search engines don\'t support base64 url, you can\'t search it.\n很抱歉，这个图片的地址是 base64 形式。由于所有的搜索引擎都不支持 base64 形式的地址，您不能搜索该图片。');
		else{
		...
		if(event.target.className=='image-search-item'&&event.button==0){
			if(event.target.getAttribute('search-option')=='all')for(var i=setting.site_option.length-1;i>=0;i--)GM_openInTab(setting.site_list[setting.site_option[i]].replace(/\{%s\}/,encodeURIComponent(img_src)));
